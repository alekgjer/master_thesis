...
def time_average_msd(x, times, dt):
    ...
    plt.plot(times, msd, label='Time averaged MSD')
    plt.plot(times, ensemble_msd, label='Ensemble MSD')
    plt.legend()
    plt.show()
...
def solve_underdamped_langevin_equation(number_of_particles, dt, t_stop):
    ...
    random_force = np.random.normal(loc=0, scale=np.sqrt(dt), size=(number_of_timesteps, number_of_particles, 3))

    # compute the Euler-Maruyama iterative scheme for the underdamped Langevin equation for all particles simultaneously
    for i in range(number_of_timesteps):
        x[i+1, :, :] = x[i, :, :] + v[i, :, :]*dt
        v[i+1, :, :] = v[i, :, :] - gamma0*v[i, :, :]*dt + np.sqrt(2*d0)*gamma0*random_force[i, :, :]
...
def solve_udsbm_langevin_equation(number_of_particles, dt, t_stop):
    ...
    for i in range(number_of_timesteps):
        x[i + 1, :, :] = x[i, :, :] + v[i, :, :] * dt
        v[i + 1, :, :] = v[i, :, :] - gamma_all_times[i] * v[i, :, :] * dt + np.sqrt(2 * d_all_times[i]) * gamma_all_times[i] * random_force[i, :, :]
